  ...
  Survey.defaultBootstrapCss.navigationButton = "btn btn-primary";
    ...
    .add(function (survey, options) {
        ...
        if (surveyId=='56' && options.question.name === "question1" && !options.question.isReadOnly) {
        	$("input[name^=question1]").click(function(){
        		if($("input[name^=question1]:checked").size()>0){
							surveyPlace = $("input[name^=question1]:checked").attr('aria-label');
        		}
        		else{
        			surveyPlace = '';
        		}

        		var appt_location = $("input[name^=question1]:checked").val();
        ...
        if (surveyId=='53' && options.question.name === "Covid_OutArea" && !options.question.isReadOnly) {
        	$("input[name^=Covid_OutArea_]").click(function(){
        		if($("input[name^=Covid_OutArea_]:checked").size()>0){
							surveyPlace = $("input[name^=Covid_OutArea_]:checked").attr('aria-label');
        		}
        		else{
        			surveyPlace = '';
        		}

        		var appt_location = $("input[name^=Covid_OutArea_]:checked").val();
    ...
    .add(function (result) {
      ...
      $.ajax({
			  ...
			  data: {'survey_id': surveyId,  'json': JSON.stringify(result.data)},
			  beforeSend: showPageLoading,
			  complete: hidePageLoading,
			  success: function(data) {
			  	if(data.error!=''){
